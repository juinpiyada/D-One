...
app.get('/', (_, res) => {
  res.json({
    status: "Server is vibing",
    poet_line: "APIs flowing like cosmic rivers, SQL humming like ancient hymns",
...
db.query('SELECT NOW()')
  .then(({ rows }) => {
    ...
    app.listen(PORT, () => {
      ...
      console.log(`🧿 Keep building. The prophecy runs in the code.`);
    });
  })
  .catch((err) => {
    console.error('🔥 Database connection failed:', err);
    console.error("⚰️ Server refusing to awaken without DB. Exiting...");
    process.exit(1);
  });
